...
public class StadiumResource {
    ...
    public List<StadiumFullInformation> getAllStadiums() {
        return this.stadiumRepository.listAll().stream()
                .map(stadium -> {
    ...
    public Response saveStadium(@PathParam("teamId") long teamId, Stadium stadium) {
        ...
        Team team = this.teamRepository.findByIdOptional(teamId)
                .orElseThrow(()-> {
    ...
    public Response updateStadiumByUpdateMethod(@PathParam("stadiumId") long stadiumId, Stadium updateStadium) {
        ...
        this.stadiumRepository.update("name = ?1, capacity = ?2 where id = ?3",
                updateStadium.getName(), updateStadium.getCapacity(), stadiumId);
    ...
    public Response updateStadiumByPersistMethod(@PathParam("stadiumId") long stadiumId, Stadium updateStadium) {
        return this.stadiumRepository.findByIdOptional(stadiumId)
                .map(stadium -> {
                ...
                .map(updatedStadium -> {
                    this.stadiumRepository.persist(updatedStadium);
                    return this.stadiumRepository.isPersistent(updatedStadium)
                            ? getStadium(updatedStadium.getName())
                            : serverError().build();
                })
    ...
    public Response deleteTeam(@PathParam("stadiumId") long stadiumId) {
        return this.stadiumRepository.findByIdOptional(stadiumId)
                .map(stadium -> {
                    boolean isDeleted = this.stadiumRepository.deleteById(stadiumId);
